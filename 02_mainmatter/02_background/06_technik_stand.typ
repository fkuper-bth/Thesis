...
Neben diesen domänenspezifischen Technologien werden auch verschiedene #utils.gls-short("cross_platform") Technologien betrachtet, die für die Implementierung dieser Arbeit in Frage kommen können.
...
Tools mit einem Fokus auf #utils.gls-short("if") generieren als Ausgabe textbasierte Spiele und bieten eine einfache Möglichkeit, interaktive Geschichten zu erstellen. Ein großer Vorteil dieser Tools ist zumeist, dass diese keine Erfahrung in der Spielentwicklung erfordern und somit Autor*innen einen einfachen Einstieg in die Welt der #utils.gls-plural("if") ermöglichen.
...
In dieser Arbeit wird sich für _#utils.gls("cmp")_ entschieden, da es eine moderne und gut unterstützte Technologie ist, die alle benötigten Funktionalitäten bietet. Kurz- und mittelfristig hängt mit der Entscheidung für _#utils.gls-short("cmp")_ auch ein gewisses Risiko zusammen, da diese Technologie noch recht jung ist (die erste Veröffentlichung war im April 2021) und sich insbesondere die Zielplattform Web noch im experimentellen Status befindet.
