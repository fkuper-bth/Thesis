...
In #ref(<kite-ii-screenshots>) sind einige Screenshots aus der KITE II Anwendung zu sehen. Dabei sieht man einmal den Start-Screen, der einerseits den Nutzer*innen graphisch aufbereitet die verschiedenen Novels zur Auswahl anbietet, diese andererseits aber auch in Form einer einfach Liste anbietet. Navigation zwischen den einzelnen Screens im Start-Screen erfolgt über eine Navigation Bar, wie man sie aus graphischen Oberflächen in mobilen Geräten kennt #ref(<navigation>).

Die Darstellung der Novels dagegen erfolgt über HTML. Die Novels werden mit Twine erstellt. Ein entsprechender Twine Compiler erstellt aus den vor-definierten Novels dann interaktive HTML-Pages. Diese können einfach mit vorhandenen mobilen UI Frameworks dargestellt werden (wie z.B. der WebView Komponente von Android #ref(<webview>)).

Die Kernelemente der vorliegenden Anwendung KITE II lassen sich also problemlos in eine alternative mobile UI Technologie wie beispielsweise Android Compose #ref(<jetpack>) oder SwiftUI #ref(<apple_xcode>) umsetzen. Im Zuge dieser Umsetzung soll ein Rahmenwerk geschaffen werden, mit dem der durch KITE II verfolgte Ansatz zur interaktiven Aufklärung und Schulung von Nutzer*innen auch mit wenig Aufwand auf andere Themengebiete angewandt werden kann.
